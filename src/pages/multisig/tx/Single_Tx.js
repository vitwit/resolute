...
import { fetchSingleMultiAccount, getMultisigAccount, getSigns, getSingleTxn, getTxn } from '../../../features/multisig/multisigSlice';
import BroadcastTx from '../BroadcastTx';
import SignTxn from '../SignTxn';

export const Single_Tx = (props) => {
    const { address, txId } = useParams();
    console.log('addressssssssssss', address, txId)
    ...
    console.log('singke sd multisig', multisigAccount)
    ...
    const isSignCountMatched = () => {
        ...
        if (threshold >= signDetails.length)
            return true
        else
            return false;
                                ...
                                textAlign={'center'}>
                                {
                                    isSignCountMatched() ?
                                        txn?.status !== 'COMPLETED' ?
                                            <BroadcastTx txId={txId}
                                                tx={txnDetails}
                                                multisigAccount={multisigAccount}
                                                signatures={signDetails} /> : null :
                                        <SignTxn
                                            txId={txId}
                        ...
                        <Table>
                            <TableBody>
                                ...
                                <TableRow>
                                    ...
                                    } {txnDetails?.fee?.amount[0].denom || 0}</strong></TableCell>
                                </TableRow>
                                <TableRow>
                                ...
                                <TableRow>
                                    ...
                                    <TableCell>: <Button>
                                        {signDetails.length}
                                            ...
                                            {Number(multisigAccount?.pubkeyJSON?.value?.threshold || 0)}
                                        </Button></TableCell>
                                </TableRow>
                                <TableRow>
                                    <TableCell>Tx Details</TableCell>
                                    <TableCell> <pre>{JSON.stringify(txnDetails?.msgs, null, 2)}</pre></TableCell>
                                </TableRow>
                                <TableRow>
                                    <TableCell>
                                        {txnDetails?.msgs[0].typeUrl}
                                    </TableCell>
                                    <TableCell colSpan={2}>
                                            ...
                                            <>
                                                {
                                                    m.typeUrl === '/cosmos.bank.v1beta1.MsgSend' ?
                                                        <span>{m?.value?.fromAddress} <strong>send </strong>  <strong>{
                                                            m?.value?.amount[0].amount + ' ' +
                                                            m?.value?.amount[0].denom
                                                        } </strong> to {m?.value?.toAddress}</span> : null
                                                }
                                                 {
                                                    m.typeUrl === '/cosmos.staking.v1beta1.MsgDelegate' ?
                                                        <span>{m?.value?.delegatorAddress} <strong> delegate </strong>  <strong>{
                                                            m?.value?.amount.amount + ' ' +
                                                            m?.value?.amount.denom
                                                        } </strong> to {m?.value?.validatorAddress}</span> : null
                                                }
                                            </>
                                        ))}
                                    </TableCell>
                                ...
                                <TableRow>
                                    ...
                                    <TableCell><pre>
                                        <strong>{signDetails && signDetails.map(s =>
                                            <span>{s.address || '-'}</span>)}</strong>
                                    </pre></TableCell>
                                </TableRow>
                            </TableBody>
                        </Table>
...
export default connect(mapStateToProps, mapDispatchToProps)(Single_Tx)