...
export default function Feegrant() {
  ...
  const [useFeeChecked, setuseFeeChecked] = React.useState(
    localStorage.getItem("fee_payer")
  );
  const handleOnFeeChecked = (event, feeInfo) => {
    setuseFeeChecked(event.target.checked);
    if (event.target.checked)
      localStorage.setItem("fee_payer", feeInfo?.granter);
    ...
    <>
      ...
      >
        <Button
          variant="contained"
          size="medium"
          onClick={() => navigateTo(`/${selectedNetwork}/feegrant/new`)}
        >
          Grant New
        </Button>
      </div>
