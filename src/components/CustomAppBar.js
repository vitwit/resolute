...
export function CustomAppBar(props) {
  ...
  const selectedNetwork = getNetworkInfo(props.selectedNetwork);
  const selectedAuthz = useSelector((state) => state.authz.selected);

  useEffect(() => {
    document.getElementById("logo-chain-main").src =
      selectedNetwork.logos?.toolbar;
    ...
    >
      {selectedAuthz.granter.length > 0 ? (
        <Toolbar
          ...
          sx={{
            minHeight: 32,
            backgroundImage:
              "linear-gradient(to right, rgb(36 133 40), rgb(4 143 34), rgb(2 120 25), rgb(7 143 4), rgb(9 191 40))",
          }}
        >
          <Typography
            component="h5"
      ...
      <Toolbar>
        ...
        <Button
          ...
          onClick={handleClick}
        >
          {selectedNetwork.config?.chainName}
        </Button>
        <Menu
        ...
        >
          {getMainNetworks().map((network, index) => (
            ...
            >
              ...
              <ListItemText>{network.config.chainName}</ListItemText>
            </MenuItem>
          ))}
