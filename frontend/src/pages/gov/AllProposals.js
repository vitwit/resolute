...
import {
  ...
  resetTx,
  txVote,
} from "../../features/gov/proposalsSlice";
...
export default function AllProposals() {
  ...
  const allChainsInfo = getMainNetworks();
  const allChainsUrls = [];
  const chainLogos = {};
  for (let i in allChainsInfo) {
    allChainsUrls.push({
      chainName: allChainsInfo[i].config.chainName,
      rest: allChainsInfo[i].config.rest,
    });
    chainLogos[allChainsInfo[i].config.chainName] = allChainsInfo[i].logos.menu;
  }
        ...
        return (
          <>
            ...
            >
              ...
              >
                <Avatar src={chainLogos[key]} alt="network-icon" />
                <Typography
                  variant="h6"
                ...
                <Badge
                  sx={{ margin: "12px" }}
                  badgeContent={proposals[key].proposals.length || 0}
                  color="primary"
                />
              </Box>
            </Box>
            ...
            <Grid key={index} container spacing={2}>
              {status === "pending" ? (
              ...
              ) : proposals[key].proposals.length === 0 ? (
                <Typography
                  variant="h6"
                  fontWeight={500}
                  color="text.primary"
                ...
                <>
                  {proposals[key].proposals.map((proposal, index) => (
