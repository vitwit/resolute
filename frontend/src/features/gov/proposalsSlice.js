...
export const getAllProposals = createAsyncThunk(
  ...
  async (data, { dispatch }) => {
    ...
    for(let j in data.baseURL){
        const response = await govService.proposals(
          ...
          if (response?.data?.proposals?.length > 0) {
            ...
            for (let i = 0; i < proposals.length; i++) {
              ...
              dispatch(
                getVotes({
                  baseURL: data.baseURL[j].chainName,
                  proposalId: proposals[i].proposal_id,
                  voter: data.voter,
                })
              );
        ...
        res[data.baseURL[j].chainName] = response.data;
    }
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(getProposalTally.fulfilled, (state, action) => {
        ...
        state.tally.proposalTally[action.payload?.tally?.proposal_id] =
          action.payload?.tally;
        state.tally.errMsg = "";
      })
    ...
    builder
      ...
      .addCase(getVotes.fulfilled, (state, action) => {
        ...
        state.votes.proposals[action.payload?.vote?.proposal_id] =
          action.payload?.vote;
        state.votes.errMsg = "";
      })
