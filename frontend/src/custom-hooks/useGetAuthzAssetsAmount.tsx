...
const useGetAuthzAssetsAmount = (chainIDs: string[]) => {
  ...
  const balanceChains = useAppSelector(
    (state: RootState) => state.bank.authz.balances
  );
  ...
  const totalAmountByChain: any = () => {
    ...
    chainIDs.forEach((chainID) => {
      ...
      for (let i = 0; i < ibcBalances?.length; i++) {
        const ibcUsdDenomPrice =
          tokensPriceInfo?.[ibcBalances?.[i]?.balance.denom]?.info?.usd || 0;
...
export default useGetAuthzAssetsAmount;
