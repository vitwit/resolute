...
const DialogTxContractStatus = ({ chainID }: { chainID: string }) => {
  ...
  const txUploadError = useAppSelector(
    (state) => state.cosmwasm.chains?.[chainID]?.txUpload?.error
  );
  const txUploadHash = useAppSelector(
    (state) => state.cosmwasm.chains?.[chainID]?.txUpload?.txHash
  );
  const txUploadedCodeId = useAppSelector(
    (state) => state.cosmwasm.chains?.[chainID]?.txUpload?.codeID
  );
        ...
        >
          <div className="w-[600px] text-white flex py-[46px] flex-col justify-center items-center">
            <div className="flex flex-col items-center gap-4  px-[60px]">
              <div>
                <Image
                  src={txUploadError ? TXN_FAILED_ICON : TXN_SUCCESS_ICON}
                  height={48}
                  width={48}
                  ...
                  <div className="truncate">
                    <div className="w-full common-copy">
                      ...
                      <Image
                        ...
                        onClick={(e) => {
                          copyToClipboard(txUploadedCodeId || '-');
                          dispatch(
                            setError({
                              type: 'success',
                              message: 'Copied',
